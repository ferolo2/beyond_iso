...
def wss(E,pvec,lp,mp,kvec,l,m):
  if l==m==lp==mp==0:
    ...
    return ( omega(k)*omega(p) - np.dot(kvec,pvec) )**2
  else:
    return 0
...
def wddpp(E,pvec,lp,mp,kvec,l,m):
  if l==m==lp==mp==0:
    k=LA.norm(kvec); p=LA.norm(pvec)
    return 1/4*( (E-omega(k))*(E-omega(p)) - np.dot(kvec,pvec) )**2
  else:
    return 0
...
def wds(E,pvec,lp,mp,kvec,l,m,Ytype):
  if lp==mp==0:
    k=LA.norm(kvec); p=LA.norm(pvec)
...
def wdd(E,pvec,lp,mp,kvec,l,m,Ytype):
  if l==m==lp==mp==0:
...
def p2sum(E,pvec,lp,mp,kvec,l,m,Ytype):
  if l==m==lp==mp==0:
...
# Finally find K3B
def K3B(E,pvec,lp,mp,kvec,l,m,Ytype):
  if l==m==lp==mp==0:
    const = -4*(2*E**2-9)
  else:
    const=0
